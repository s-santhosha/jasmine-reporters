...
(function(global) {
  ...
  exportObject.JUnitXmlReporter = function(options) {
    ...
    self.consolidate =
      options.consolidate === UNDEFINED ? true : options.consolidate;
    self.consolidateAll =
      self.consolidate !== false &&
      (options.consolidateAll === UNDEFINED ? true : options.consolidateAll);
    self.useDotNotation =
      options.useDotNotation === UNDEFINED ? true : options.useDotNotation;
    self.useFullTestName =
      options.useFullTestName === UNDEFINED ? false : options.useFullTestName;
    if (self.consolidateAll) {
      self.filePrefix = options.filePrefix || "junitresults";
    } else {
      self.filePrefix =
        typeof options.filePrefix === "string"
          ? options.filePrefix
          : "junitresults-";
    }
    self.package =
      typeof options.package === "string"
        ? escapeInvalidXmlChars(options.package)
        : UNDEFINED;
    self.stylesheetPath =
      (typeof options.stylesheetPath === "string" && options.stylesheetPath) ||
      UNDEFINED;

    if (
      options.modifySuiteName &&
      typeof options.modifySuiteName !== "function"
    ) {
      throw new Error('option "modifySuiteName" must be a function');
    }
    if (
      options.modifyReportFileName &&
      typeof options.modifyReportFileName !== "function"
    ) {
    ...
    self.formatSuiteData = function(suite) {
      return {
        ...
        timestamp: ISODateString(suite._startTime)
      };
    };
    ...
    self.getOrWriteNestedOutput = function(suite) {
      ...
      if (self.consolidateAll || (self.consolidate && suite._parent)) {
        ...
        wrapOutputAndWriteFile(
          generateFilename(suite),
          output,
          self.getNestedSuiteData(suite)
        );
        return "";
      }
    };
    ...
    self.writeFile = function(filename, text) {
      ...
      try {
        ...
        errors.push("  PhantomJs attempt: " + e.message);
      }
      try {
        nodeWrite(path, filename, text);
        return;
      } catch (f) {
        errors.push("  NodeJS attempt: " + f.message);
      }

      // If made it here, no write succeeded.  Let user know.
      log(
        "Warning: writing junit report failed for '" +
          path +
          "', '" +
          filename +
          "'. Reasons:\n" +
    ...
    function specAsXml(spec) {
      ...
      var xml =
        '\n  <testcase classname="' +
        getFullyQualifiedSuiteName(spec._suite) +
        '"';
      ...
      if (isSkipped(spec) || isDisabled(spec)) {
        if (spec.pendingReason) {
          testCaseBody =
            '\n   <skipped message="' +
            trim(escapeInvalidXmlChars(spec.pendingReason)) +
            '" />';
        } else {
          testCaseBody = "\n   <skipped />";
        }
        ...
        for (var i = 0, failure; i < spec.failedExpectations.length; i++) {
          ...
          testCaseBody +=
            ' message="' + trim(escapeInvalidXmlChars(failure.message)) + '"';
          testCaseBody += ">";
          testCaseBody +=
            "<![CDATA[" +
            trim(escapeControlChars(failure.stack || failure.message)) +
            "]]>";
          testCaseBody += "\n   </failure>";
        }
      }
          ...
          xml +=
            ...
            trim(
              escapeInvalidXmlChars(
                delegates.systemOut(
                  spec,
                  getFullyQualifiedSuiteName(spec._suite, true)
                )
              )
            ) +
            "</system-out>";
